...
abstract class GuiElement {
    ...
    abstract fun render(drawContext: GuiGraphics, screenWidth: Int, screenHeight: Int, alpha: Float)

    fun onRender2D(event: Render2DEvent) {
        if (!active) return
        val alpha = getAlpha()
        if (alpha <= 0f) return
        render(event.matrices, mc.window.guiScaledWidth, mc.window.guiScaledHeight, alpha)
    }
    ...
    protected fun drawTexture(
        ...
        if (texture !in registeredTextures) {
            ...
            if (stream != null) {
                ...
                mc.textureManager.register(texture, DynamicTexture({ texture.toString() }, nativeImage))
                stream.close()
            }
    ...
    companion object {
        private val registeredTextures = mutableSetOf<ResourceLocation>()
    }
    ...
    protected fun drawCenteredText(
        ...
        val textWidth = mc.font.width(text) * scale
