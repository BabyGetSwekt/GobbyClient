...
object HudDrawing {
    ...
    fun drawPanelBackground(ctx: GuiGraphicsExtractor, width: Int, height: Int) {
        ctx.fill(0, 0, width, height, BACKGROUND_COLOR)
    }

    fun drawBoxWithBorder(ctx: GuiGraphicsExtractor, x: Int, y: Int, width: Int, height: Int) {
        ctx.fill(x - 1, y - 1, x + width + 1, y + height + 1, BORDER_COLOR)
        ctx.fill(x, y, x + width, y + height, INNER_COLOR)
    }

    fun drawSolidBox(ctx: GuiGraphicsExtractor, x: Int, y: Int, width: Int, height: Int, fillColor: Int) {
        ctx.fill(x - 1, y - 1, x + width + 1, y + height + 1, BORDER_COLOR)
        ctx.fill(x, y, x + width, y + height, fillColor)
    }

    fun drawOutline(ctx: GuiGraphicsExtractor, x: Int, y: Int, width: Int, height: Int, color: Int) {
    ...
    fun drawCenteredText(ctx: GuiGraphicsExtractor, text: String, x: Int, y: Int, width: Int, height: Int, color: Int = TEXT_COLOR) {
        val font = mc.font
        val textWidth = font.width(text)
        val textX = x + (width - textWidth) / 2
        val textY = y + (height - font.lineHeight) / 2 + 1
        ctx.text(font, text, textX, textY, color, true)
    }
}
