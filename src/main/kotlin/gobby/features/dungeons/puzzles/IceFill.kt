...
object IceFill : Module("Ice Fill", "Solves (and auto-completes) Ice Fill puzzle in F7", Category.DUNGEONS) {
    ...
    internal enum class Floor(val y: Int, val xMin: Int, val xMax: Int, val zMin: Int, val zMax: Int, val start: BlockPos, val exit: BlockPos, val color: Color) {
    ...
    private fun reset() { path = null; lastFiredIndex = -1; MovementPacketSuppressor.clear(); solving = false }
    ...
    fun onRoomEnter(event: RoomEnterEvent) {
        ...
        val masks = Floor.entries.map { it to IceFillSolver.buildMask(it, room, world) }
        solving = true
        val timer = Clock()
        thread(name = "GobbyClient-IceFill", isDaemon = true) {
            try {
                val solved = masks.map { (floor, mask) -> floor to IceFillSolver.solve(floor, mask) }
